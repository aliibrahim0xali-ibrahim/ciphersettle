...
//!
//! This exists so `ciphersettle_core` can compute the invoice nullifier
//! on-chain (see `nullifier.rs`) without pulling in an external crate --
//! keeping this crate buildable on any Rust toolchain, including the
//! Ubuntu-packaged rustc used to test this project. It is deliberately
//! small and unoptimized; it is not a general-purpose hashing library.
//!
//! Correctness is checked against the NIST test vectors in the `tests`
//! module below (the empty string, "abc", and the standard two-block
//! message), not merely asserted.
